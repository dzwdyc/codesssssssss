...
FlutteringFairy* FlutteringFairy::addFairy(cocos2d::Node *parent, cocos2d::Point point, FairyType type, int value, Ref* target, SEL_CallFunc callfunc)
{
    FlutteringFairy* fairy = new FlutteringFairy();
    fairy->initWithFairy(type, value, target, callfunc);
    fairy->setPosition(parent->getPosition()+ point);
